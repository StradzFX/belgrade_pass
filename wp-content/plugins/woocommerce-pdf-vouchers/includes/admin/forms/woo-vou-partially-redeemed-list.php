...
class WOO_Vou_Partially_Redeem extends WP_List_Table {
	...
	function display_used_vouchers() {
		...
		if( isset( $_GET['voucherid'] ) && !empty( $_GET['voucherid'] ) ) {
			...
			$voucherid_args= array( 'post_parent' => $_GET['voucherid'] );
			$args =	array_merge( $args,	$voucherid_args );				
		}
		...
		$woo_data 	= $this->model->woo_vou_get_partially_redeem_details( $args );
		...
		if( !empty( $data ) ) {
			...
			foreach ( $data as $key => $value ) {
				...
				$data[$key]['code'] 		= get_post_meta( $data[$key]['post_parent'], $prefix.'purchased_codes', true );
				$data[$key]['redeem_by'] 	= $display_name;
				$data[$key]['redeem_amount'] 	= get_post_meta( $value['ID'], $prefix.'partial_redeem_amount', true );
				$data[$key]['first_name'] 	= get_post_meta( $data[$key]['post_parent'], $prefix.'first_name', true );
				$data[$key]['last_name'] 	= get_post_meta( $data[$key]['post_parent'], $prefix.'last_name', true );
				$data[$key]['order_id'] 	= get_post_meta( $data[$key]['post_parent'], $prefix.'order_id', true );
				$data[$key]['order_date'] 	= get_post_meta( $data[$key]['post_parent'], $prefix.'order_date', true );
	...
	function column_default( $item, $column_name ) {
		...
		switch( $column_name ) {
			...
			case 'buyers_info' :
				...
				break;
			case 'redeem_by' :
				$column_value = $this->model->woo_vou_display_redeem_info_html( $item['ID'], $item['order_id'] ,'',$defualt= 'partially');
				break;
			case 'product_info' :
	...
	function no_items() {
		...
		_e( 'No partially used voucher codes yet.', 'woovoucher' );
	}
...
$WooPartiallyUsedVouListTable = new WOO_Vou_Partially_Redeem();

//Fetch, prepare, sort, and filter our data...
$WooPartiallyUsedVouListTable->prepare_items();?>
...
<div class="wrap"><?php 
	...
	$WooPartiallyUsedVouListTable->views();?>
    ...
    <form id="product-filter" method="get" action="">
        ...
        <!-- Search Title -->
        <?php $WooPartiallyUsedVouListTable->search_box( __( 'Search', 'woovoucher' ), 'woovoucher' ); ?>

       <div class="alignright">
        <?php
			$generatpdfurl = add_query_arg( array( 
													'woo-vou-voucher-gen-pdf'	=> '1',
													'woo_vou_action'			=> 'partially'
												));
        ...
        <?php $WooPartiallyUsedVouListTable->display(); ?>
    </form>
</div>