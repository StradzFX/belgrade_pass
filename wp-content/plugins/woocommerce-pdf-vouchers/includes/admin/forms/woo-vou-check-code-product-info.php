...
if( $enable_partial_redeem == "yes" ) {
	...
	$vou_code_remaining_redeem_price = $vou_code_total_price - $vou_code_total_redeemed_price;			
}
?>
						...
						switch ( $col_key ) {
							...
							case 'logo' :									
								if( !empty(  $allvoucherdata['vendor_logo']['src'] ) )
									$column_value .= '<div class="woo_pdf_res_vou"> Logo </div>' . '<img src="' . $allvoucherdata['vendor_logo']['src'] . '" alt="" width="70" height="70" />';
								else 
									$column_value .= '<div class="woo_pdf_res_vou"> Logo </div> &nbsp;';
								break;
							case 'voucher_data' : 
								ob_start(); ?>									
								...
								$column_value = '<div class="woo_pdf_res_vou"> Voucher Data </div> <div class="woo_pdf_val">'. ob_get_clean() . '</div>';
								break;
							case 'expires' :
								$column_value = '<div class="woo_pdf_res_vou"> 	Expires </div>'; 
				...
				foreach ( $buyer_info_columns as $col_key => $column ) { ?>
					...
					<div class="<?php echo $buyer_col; ?> woo_vou_padding">
						...
						switch ( $col_key ) { 
							...
							case 'buyer_name' : 
								$column_value .= '<div class="woo_pdf_res_buyer"> Name </div>' . $order->billing_first_name;
							...
							case 'buyer_email' : 
								$column_value .='<div class="woo_pdf_res_buyer"> Email </div>' .$order->billing_email;
								break;
							
							case 'billing_address' : 
								$column_value .= '<div class="woo_pdf_res_buyer"> Billing Address </div> <div class="woo_pdf_val">' . $billing_address . "</div>";
								break;
							
							case 'shipping_address' : 
								$column_value .= '<div class="woo_pdf_res_buyer"> Shipping Address </div> <div class="woo_pdf_val">' . $shipping_address . '</div>';
								break;
							
							case 'buyer_phone' : 
								$column_value .= '<div class="woo_pdf_res_buyer"> Phone </div>' . $order->billing_phone;
								break;
				...
				foreach ( $order_info_columns as $col_key => $column ) {?>
					...
					<div class="<?php echo $order_col; ?> woo_vou_padding"><?php
						...
						switch ( $col_key ) { 
							...
							case 'order_id' :
								$column_value .= '<div class="woo_pdf_res_order"> Order ID </div>'. $order_id;
								break;
							
							case 'order_date' :
								$column_value .=  '<div class="woo_pdf_res_order"> Order Date </div>' . $this->model->woo_vou_get_date_format( $order->post->post_date, true );
								break;
							
							case 'payment_method' : 
								$column_value .=  '<div class="woo_pdf_res_order"> Payment Method </div>' . $payment_method;
								break;
							
							case 'order_total':
								$column_value .=  '<div class="woo_pdf_res_order"> Order Total </div>' . esc_html( strip_tags( $order->get_formatted_order_total() ) );
								break;
							
							case 'order_discount' : 
								$column_value .=  '<div class="woo_pdf_res_order"> Order Discount </div>' . wc_price( $order->get_total_discount(), array( 'currency' => $order->get_order_currency() ) );
								break;
		...
		<div class="woo_pdf_vou_main">
			...
			<div class="woo_pdf_vou_pro_lst woo-vou-partial-redeem-amount woo-vou-margin-bottom-0">
				...
				<div class="col-2 woo_vou_padding">
					...
					<span class="description"><?php _e( 'Enter the amount you want to redeem.', 'wooovoucher' ); ?></span>
