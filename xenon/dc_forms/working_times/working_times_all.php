...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$working_times->checker = $checker;
		$working_times->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($working_times)) as $name => $value)
		{
			$working_times->$name = addslashes($working_times->$name);
		}
		if($broker->update($working_times,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"working_times",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new working_times($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"working_times","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$working_times = $broker->get_data(new working_times($first->id));
				$working_times->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($working_times)) as $name => $value)
				{
					$working_times->$name = addslashes($working_times->$name);	
				}
				$broker->update($working_times);
						
				$working_times = $broker->get_data(new working_times($second->id));
				$working_times->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($working_times)) as $name => $value)
				{
					$working_times->$name = addslashes($working_times->$name);		
				}
...
$_SESSION["export_dc"] = "working_times";
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
	...
	$all_dc_objects[$i]->ts_location = $all_dc_objects[$i]->ts_location->training_school;	
...
$sql = $broker->execute_query("select distinct `maker` from `working_times` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Working times</h1>
