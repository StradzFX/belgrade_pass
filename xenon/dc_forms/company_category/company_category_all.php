...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$company_category->checker = $checker;
		$company_category->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($company_category)) as $name => $value)
		{
			$company_category->$name = addslashes($company_category->$name);
		}
		if($broker->update($company_category,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_category",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new company_category($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_category","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$company_category = $broker->get_data(new company_category($first->id));
				$company_category->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($company_category)) as $name => $value)
				{
					$company_category->$name = addslashes($company_category->$name);	
				}
				...
				$company_category = $broker->get_data(new company_category($second->id));
				$company_category->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($company_category)) as $name => $value)
				{
					$company_category->$name = addslashes($company_category->$name);		
				}
...
$_SESSION["export_dc"] = "company_category";
...
$sql = $broker->execute_query("select distinct `maker` from `company_category` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Company category</h1>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		$sort="category";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Category</a>
		</th>
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		<th width="64">
		<?php 
		...
		<td><?php echo $all_dc_objects[$i]->category; ?></td>
		<?php include_once "custom_td.php";?>
