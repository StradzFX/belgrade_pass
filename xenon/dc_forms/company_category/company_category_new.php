...
//================ ERROR HANDLER - INPUT|INPUT READONLY|DROPMENUEXT|DROPMENUPARENT ELEMENT - Category

	if(isset($category))
	{
		if(!is_numeric($category) && $category != "NULL")
			$error_message = $ap_lang["Field"] . " Category " . $ap_lang["must be number!"];
		
		if(strlen($category) > 11)
			$error_message = $ap_lang["Field"] . " Category " . $ap_lang["must be below"] . " 11 ". $ap_lang["characters!"];
	}
	...
	if($error_message == "")
	{
		if($_POST["promote"] == "yes"){
			...
			$checker = "";
			$checkerDate = $company_category->checkerDate;
		...
		$new_object = new company_category();
		$new_object->company = $company;$new_object->category = $category;$new_object->maker = $_SESSION[ADMINLOGGEDIN];
		...
		if($new_dc_id > 0)
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_category","new",$new_dc_id));
...
if($error_message != "")
{
	...
	foreach($_POST as $key => $value)
		$company_category->$key = htmlentities(stripslashes($_POST[$key]),ENT_COMPAT,"UTF-8");
	
	if(isset($_POST["promote"]))	$company_category->checker = $_SESSION[ADMINLOGGEDIN];
	else							$company_category->checker = "";
}
else	unset($company_category);
?>
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["New"]; ?> - Company category</h1>
...
<tr>
	...
	<td>
	<?php if($company_category->checker != ""){ ?>
