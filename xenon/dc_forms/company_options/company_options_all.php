...
if($filter_tag != "all" && $filter_tag != "")
	$dc_objects->add_condition("tag","=",$filter_tag);
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$company_options->checker = $checker;
		$company_options->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($company_options)) as $name => $value)
		{
			$company_options->$name = addslashes($company_options->$name);
		}
		if($broker->update($company_options,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_options",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new company_options($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_options","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$company_options = $broker->get_data(new company_options($first->id));
				$company_options->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($company_options)) as $name => $value)
				{
					$company_options->$name = addslashes($company_options->$name);	
				}
				$broker->update($company_options);
						
				$company_options = $broker->get_data(new company_options($second->id));
				$company_options->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($company_options)) as $name => $value)
				{
					$company_options->$name = addslashes($company_options->$name);		
				}
...
$_SESSION["export_dc"] = "company_options";
...
$sql = $broker->execute_query("select distinct `maker` from `company_options` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Company options</h1>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		$sort="company";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Company</a>
		</th>
		<th width="250">
		<?php 
		if($sort_column=="tag")
		...
		$sort="tag";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Tag</a>
		</th>
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		<th width="64">
		<?php 
		...
		<td><?php echo $all_dc_objects[$i]->company; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->tag; ?></td>
		<?php include_once "custom_td.php";?>
