...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$website_lang->checker = $checker;
		$website_lang->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($website_lang)) as $name => $value)
		{
			$website_lang->$name = addslashes($website_lang->$name);
		}
		if($broker->update($website_lang,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"website_lang",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new website_lang($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"website_lang","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$website_lang = $broker->get_data(new website_lang($first->id));
				$website_lang->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($website_lang)) as $name => $value)
				{
					$website_lang->$name = addslashes($website_lang->$name);	
				}
				$broker->update($website_lang);
						
				$website_lang = $broker->get_data(new website_lang($second->id));
				$website_lang->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($website_lang)) as $name => $value)
				{
					$website_lang->$name = addslashes($website_lang->$name);		
				}
...
$_SESSION["export_dc"] = "website_lang";
...
$sql = $broker->execute_query("select distinct `maker` from `website_lang` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Website language</h1>
