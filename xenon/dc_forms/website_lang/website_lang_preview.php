...
require_once "../classes/domain/website_lang.class.php";
...
$website_lang = $broker->get_data(new website_lang($_GET["id"]));
...
//================ SEARCH FILTER INITIALIZATION 	
	...
	$array_som[] = array("shortname","LIKE","%".$search."%","OR");
...
if($filter_shortname != "all" && $filter_shortname != "")
	$dc_objects->add_condition("shortname","=",$filter_shortname);

$dc_objects->add_condition("jezik","=",$filter_lang);
$dc_objects->set_order_by("pozicija","DESC");

$min_item = new website_lang();
...
$sec_to_min = new website_lang();
$sec_to_min->condition = $dc_objects->condition;
$sec_to_min->order_by = $dc_objects->order_by;
$sec_to_min->add_condition("pozicija","<",$website_lang->pozicija);
...
$sec_to_max = new website_lang();
$sec_to_max->condition = $dc_objects->condition;
$sec_to_max->order_by = $dc_objects->order_by;
$sec_to_max->add_condition("pozicija",">",$website_lang->pozicija);
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["Preview"]; ?> - Website language</h1>
...
<!--FORM TYPE HIDDEN-->
<input type="hidden" name="id" value="<?php echo $website_lang->id; ?>"/>
<!--FORM TYPE INPUT-->
<script>
...
<td>
	<?php 
		if($_GET["action"] == "preview"){ 
			echo $website_lang->name; 
		}else{ 
	?>
	<input type="text" name="name" value="<?php echo $website_lang->name; ?>" style="width:600px;" limit="250" onkeyup="count_input_limit('name')">
	<?php } ?>
</td>
</tr>
<!--FORM TYPE INPUT-->
<script>
	...
	$(function(){
		count_input_limit("shortname");
		
	});
</script>
<tr>
<td>Shortname <span id="shortname_counter" style="color:#999">(250)</span></td>
	
<td>
	<?php 
		if($_GET["action"] == "preview"){ 
			echo $website_lang->shortname; 
		}else{ 
	?>
	<input type="text" name="shortname" value="<?php echo $website_lang->shortname; ?>" style="width:600px;" limit="250" onkeyup="count_input_limit('shortname')">
	<?php } ?>
</td>
</tr>	
<?php if($website_lang->maker != ""){ ?>
<tr>
	<td><?php echo $ap_lang["Maker"]; ?></td>
	<td>
	<?php  echo $website_lang->maker; ?> (<?php  echo $website_lang->makerDate; ?>)
	</td>
</tr>
<?php } ?>
<?php if($website_lang->checker != "" && $error_message == ""){ ?>
<tr>
	<td><?php echo $ap_lang["Checker"]; ?></td>
	<td>
	<?php  echo $website_lang->checker; ?> (<?php  echo $website_lang->checkerDate; ?>)
	</td>
</tr>
<?php } ?>
</table>
