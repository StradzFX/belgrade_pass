...
//================ ERROR HANDLER - INPUT|INPUT READONLY|DROPMENUEXT|DROPMENUPARENT ELEMENT - Shortname
	...
	if($error_message == "")
	{
		...
		$new_object = new website_lang();
		$new_object->name = $name;$new_object->shortname = $shortname;$new_object->maker = $_SESSION[ADMINLOGGEDIN];
		...
		if($new_dc_id > 0)
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"website_lang","new",$new_dc_id));
...
if($error_message != "")
{
	$website_lang = new website_lang();
	foreach($_POST as $key => $value)
		$website_lang->$key = htmlentities(stripslashes($_POST[$key]),ENT_COMPAT,"UTF-8");
	
	if(isset($_POST["promote"]))	$website_lang->checker = $_SESSION[ADMINLOGGEDIN];
	else							$website_lang->checker = "";
}
else	unset($website_lang);
?>
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["New"]; ?> - Website language</h1>
