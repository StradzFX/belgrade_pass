...
//================ ERROR HANDLER - INPUT|INPUT READONLY|DROPMENUEXT|DROPMENUPARENT ELEMENT - Pass Per_kid
	...
	if($error_message == "")
	{
		...
		if($new_dc_id > 0)
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"cl_pass_rules","new",$new_dc_id));
...
if($error_message != "")
{
	$cl_pass_rules = new cl_pass_rules();
	foreach($_POST as $key => $value)
		$cl_pass_rules->$key = htmlentities(stripslashes($_POST[$key]),ENT_COMPAT,"UTF-8");
	
	if(isset($_POST["promote"]))	$cl_pass_rules->checker = $_SESSION[ADMINLOGGEDIN];
	else							$cl_pass_rules->checker = "";
}
else	unset($cl_pass_rules);
?>
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["New"]; ?> - cl_pass_rules</h1>
