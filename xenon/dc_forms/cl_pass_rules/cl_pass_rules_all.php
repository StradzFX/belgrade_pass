...
//================ PHP HANDLER FOR DROPMENUPARENT CLASS ts_location
...
if($_GET["filter_training_school"] != "all" && $_GET["filter_training_school"] != NULL)
	$ts_location->add_condition("training_school","=",$_GET["filter_training_school"]);
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$cl_pass_rules->checker = $checker;
		$cl_pass_rules->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($cl_pass_rules)) as $name => $value)
		{
			$cl_pass_rules->$name = addslashes($cl_pass_rules->$name);
		}
		if($broker->update($cl_pass_rules,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"cl_pass_rules",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new cl_pass_rules($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"cl_pass_rules","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$cl_pass_rules = $broker->get_data(new cl_pass_rules($first->id));
				$cl_pass_rules->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($cl_pass_rules)) as $name => $value)
				{
					$cl_pass_rules->$name = addslashes($cl_pass_rules->$name);	
				}
				$broker->update($cl_pass_rules);
						
				$cl_pass_rules = $broker->get_data(new cl_pass_rules($second->id));
				$cl_pass_rules->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($cl_pass_rules)) as $name => $value)
				{
					$cl_pass_rules->$name = addslashes($cl_pass_rules->$name);		
				}
...
$_SESSION["export_dc"] = "cl_pass_rules";
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
	...
	$all_dc_objects[$i]->ts_location = $all_dc_objects[$i]->ts_location->part_of_city;	
...
$sql = $broker->execute_query("select distinct `maker` from `cl_pass_rules` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - cl_pass_rules</h1>
	...
	<fieldset>
		...
		<select name="filter_ts_location" onchange="submit()">
			<option value="all"><?php echo $ap_lang["All"]; ?></option>
			<?php if($filter_training_school != "all" && $filter_training_school != NULL) { ?> 
 		<?php for($i=0; $i < sizeof($all_ts_location); $i++) { ?> 
			<option value="<?php echo $all_ts_location[$i]->id; ?>" <?php if($filter_ts_location == $all_ts_location[$i]->id){ ?>selected="selected"<?php } ?>><?php echo $all_ts_location[$i]->part_of_city; ?></option>
        <?php } ?>
		<?php } else{$filter_ts_location = "all"; }?>
		</select>
		</div>
