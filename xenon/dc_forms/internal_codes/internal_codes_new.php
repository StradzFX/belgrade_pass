...
//================ ERROR HANDLER - INPUT|INPUT READONLY|DROPMENUEXT|DROPMENUPARENT ELEMENT - Code Value
	...
	if($error_message == "")
	{
		...
		$new_object = new internal_codes();
		$new_object->code_name = $code_name;$new_object->training_school = $training_school;$new_object->code_value = $code_value;$new_object->maker = $_SESSION[ADMINLOGGEDIN];
		...
		if($new_dc_id > 0)
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"internal_codes","new",$new_dc_id));
...
if($error_message != "")
{
	...
	if(isset($_POST["promote"]))	$internal_codes->checker = $_SESSION[ADMINLOGGEDIN];
	else							$internal_codes->checker = "";
}
else	unset($internal_codes);
?>
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["New"]; ?> - Internal codes</h1>
