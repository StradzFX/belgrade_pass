...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$internal_codes->checker = $checker;
		$internal_codes->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($internal_codes)) as $name => $value)
		{
			$internal_codes->$name = addslashes($internal_codes->$name);
		}
		if($broker->update($internal_codes,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"internal_codes",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new internal_codes($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"internal_codes","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$internal_codes = $broker->get_data(new internal_codes($first->id));
				$internal_codes->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($internal_codes)) as $name => $value)
				{
					$internal_codes->$name = addslashes($internal_codes->$name);	
				}
				$broker->update($internal_codes);
						
				$internal_codes = $broker->get_data(new internal_codes($second->id));
				$internal_codes->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($internal_codes)) as $name => $value)
				{
					$internal_codes->$name = addslashes($internal_codes->$name);		
				}
...
$_SESSION["export_dc"] = "internal_codes";
...
$sql = $broker->execute_query("select distinct `maker` from `internal_codes` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Internal codes</h1>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		<th width="250">
		<?php 
		if($sort_column=="code_name")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="code_name";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Code Name</a>
		</th>
		<th width="250">
		<?php 
	...
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		...
		<td><?php echo $all_dc_objects[$i]->code_name; ?></td>
		<?php include_once "custom_td.php";?>
