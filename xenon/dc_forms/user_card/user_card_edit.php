...
if($_SESSION[ADMINMAKER]==1 && $_SESSION[ADMINCHECKER]==0 && $user_card->checker != "")
{
...
//=================================================================================================
	...
	if($error_message == "")
	{
		...
		if($broker->update(new user_card($id,$card_number,$child_birthdate,$user,$card_password,$delivery_method,$post_street,$post_city,$post_postal,$partner_id,$customer_received,$parent_first_name,$number_of_kids,$city,$phone,$email,$company_location,$parent_last_name,$user_card->maker,$user_card->makerDate,$checker,$checkerDate,$user_card->pozicija,$user_card->jezik,$user_card->recordStatus,$user_card->modNumber+1,$user_card->multilang_id)) == 1)
		{
			$success_message = $ap_lang["This object was edited successfully!"];
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"user_card","edit",$_GET["id"]));
		}
...
foreach($user_card as $key => $value)
	$user_card->$key = htmlentities($user_card->$key,ENT_COMPAT,"UTF-8");
...
if(isset($_POST["promote"]) && $user_card->checker == "")	
	$user_card->checker = $_SESSION[ADMINLOGGEDIN];
?>
<script src="js/wledit/wledit.js" type="text/javascript"></script>
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["Edit"]; ?> - User card</h1>
