...
if($_SESSION[ADMINMAKER]==1 && $_SESSION[ADMINCHECKER]==0 && $training_school->checker != "")
{
...
//================ POST VARIABLES INITIALIZATION
	...
	$pass_options = $_POST["pass_options"];
...
//=================================================================================================
	...
	if($error_message == "")
	{
		...
		if($broker->update(new training_school($id,$name,$into_text,$general_text,$sport_category,$featured,$number_of_views,$short_description,$main_description,$promoted,$username,$password,$pass_options,$extra_goods_options,$birthday_options,$pass_customer_percentage,$pass_company_percentage,$discount_description,$training_school->maker,$training_school->makerDate,$checker,$checkerDate,$training_school->pozicija,$training_school->jezik,$training_school->recordStatus,$training_school->modNumber+1,$training_school->multilang_id)) == 1)
		{
			$success_message = $ap_lang["This object was edited successfully!"];
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"training_school","edit",$_GET["id"]));
		}
		else	$error_message = $ap_lang["There was an error while editing data!"];
	}
}
$training_school = $broker->get_data(new training_school($_GET["id"]));
foreach($training_school as $key => $value)
	$training_school->$key = htmlentities($training_school->$key,ENT_COMPAT,"UTF-8");
...
if(isset($_POST["promote"]) && $training_school->checker == "")	
	$training_school->checker = $_SESSION[ADMINLOGGEDIN];
?>
<script src="js/wledit/wledit.js" type="text/javascript"></script>
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["Edit"]; ?> - Training school</h1>
