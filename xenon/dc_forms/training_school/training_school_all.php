...
if($_GET)
{
	...
	$filter_sport_category = $_GET["filter_sport_category"];
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$training_school->checker = $checker;
		$training_school->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($training_school)) as $name => $value)
		{
			$training_school->$name = addslashes($training_school->$name);
		}
		if($broker->update($training_school,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"training_school",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new training_school($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"training_school","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$training_school = $broker->get_data(new training_school($first->id));
				$training_school->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($training_school)) as $name => $value)
				{
					$training_school->$name = addslashes($training_school->$name);	
				}
				$broker->update($training_school);
						
				$training_school = $broker->get_data(new training_school($second->id));
				$training_school->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($training_school)) as $name => $value)
				{
					$training_school->$name = addslashes($training_school->$name);		
				}
...
$_SESSION["export_dc"] = "training_school";
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
	...
	if($all_dc_objects[$i]->sport_category != NULL){
		
		$all_dc_objects[$i]->sport_category = $broker->get_data(new sport_category($all_dc_objects[$i]->sport_category));
	$all_dc_objects[$i]->sport_category = $all_dc_objects[$i]->sport_category->name;	
	
	}else{
		$all_dc_objects[$i]->sport_category = "";
	}
...
$sql = $broker->execute_query("select distinct `maker` from `training_school` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Training school</h1>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		<th width="167">
		<?php 
		if($sort_column=="discount_description")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="discount_description";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Discount Description</a>
		</th>
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		<th width="64">
		<?php 
		...
		<td><?php echo $all_dc_objects[$i]->discount_description; ?></td>
		<?php include_once "custom_td.php";?>
