...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$ts_trainers->checker = $checker;
		$ts_trainers->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($ts_trainers)) as $name => $value)
		{
			$ts_trainers->$name = addslashes($ts_trainers->$name);
		}
		if($broker->update($ts_trainers,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"ts_trainers",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new ts_trainers($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"ts_trainers","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$ts_trainers = $broker->get_data(new ts_trainers($first->id));
				$ts_trainers->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($ts_trainers)) as $name => $value)
				{
					$ts_trainers->$name = addslashes($ts_trainers->$name);	
				}
				$broker->update($ts_trainers);
						
				$ts_trainers = $broker->get_data(new ts_trainers($second->id));
				$ts_trainers->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($ts_trainers)) as $name => $value)
				{
					$ts_trainers->$name = addslashes($ts_trainers->$name);		
				}
...
$_SESSION["export_dc"] = "ts_trainers";
...
$sql = $broker->execute_query("select distinct `maker` from `ts_trainers` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - TS Trainers</h1>
