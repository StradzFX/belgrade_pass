...
if($_GET)
{
	...
	$filter_best_value = $_GET["filter_best_value"];
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$card_package->checker = $checker;
		$card_package->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($card_package)) as $name => $value)
		{
			$card_package->$name = addslashes($card_package->$name);
		}
		if($broker->update($card_package,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"card_package",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new card_package($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"card_package","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$card_package = $broker->get_data(new card_package($first->id));
				$card_package->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($card_package)) as $name => $value)
				{
					$card_package->$name = addslashes($card_package->$name);	
				}
				$broker->update($card_package);
						
				$card_package = $broker->get_data(new card_package($second->id));
				$card_package->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($card_package)) as $name => $value)
				{
					$card_package->$name = addslashes($card_package->$name);		
				}
...
$_SESSION["export_dc"] = "card_package";

for($i=0; $i<sizeof($all_dc_objects); $i++)
{
	
	
		if($all_dc_objects[$i]->best_value == "1")	$all_dc_objects[$i]->best_value = 1;
		else														$all_dc_objects[$i]->best_value = 0;
...
$sql = $broker->execute_query("select distinct `maker` from `card_package` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Card Package</h1>
	...
	<fieldset>
		...
		<select name="filter_best_value" onchange="submit()">
			<option value="all"><?php echo $ap_lang["All"]; ?></option>
 			<option value="yes" <?php if($filter_best_value == "yes"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["Yes"]; ?></option>
        	<option value="no" <?php if($filter_best_value == "no"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["No"]; ?></option>
		</select>
		</div>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		<th width="63">
		<?php 
		if($sort_column=="to_company")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="to_company";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">To Company</a>
		</th>
		<th width="63">
		<?php 
		if($sort_column=="to_us")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="to_us";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">To Us</a>
		</th>
		<th width="63">
		<?php 
		if($sort_column=="duration_days")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="duration_days";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Duration Days</a>
		</th>
		<th width="63">
		<?php 
		if($sort_column=="number_of_passes")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="number_of_passes";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Number Of Passes</a>
		</th>
		<th width="63">
		<?php 
		if($sort_column=="best_value")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="best_value";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Best Value</a>
		</th>
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		<th width="64">
		<?php 
...
<td><?php if($all_dc_objects[$i]->picture != ""){ ?><a id="picture" href="../pictures/card_package/picture/<?php echo $all_dc_objects[$i]->picture; ?>"><img src="../pictures/card_package/picture/<?php echo $all_dc_objects[$i]->picture; ?>" alt="<?php echo $all_dc_objects[$i]->picture; ?>" title="<?php echo $all_dc_objects[$i]->picture; ?>" border="0"/></a>
<?php } ?></td>
		<td><?php echo $all_dc_objects[$i]->price; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->to_company; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->to_us; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->duration_days; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->number_of_passes; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php if($all_dc_objects[$i]->best_value == 1){ ?><img src="images/checked.png" alt="<?php echo $ap_lang["Yes"]; ?>" title="<?php echo $ap_lang["Yes"]; ?>" border="0" /> <?php }else{ ?><img src="images/not_checked.png" alt="<?php echo $ap_lang["No"]; ?>" title="<?php echo $ap_lang["No"]; ?>" border="0" /><?php } ?></td>
<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
