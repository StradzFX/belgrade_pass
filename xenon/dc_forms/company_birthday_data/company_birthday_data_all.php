...
//================ PHP HANDLER FOR DROPMENUEXT CLASS ts_location	
...
if($_GET)
{
	...
	$filter_smoking = $_GET["filter_smoking"];
	$filter_catering = $_GET["filter_catering"];
	$filter_animators = $_GET["filter_animators"];
	$filter_watching_kids = $_GET["filter_watching_kids"];
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$company_birthday_data->checker = $checker;
		$company_birthday_data->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($company_birthday_data)) as $name => $value)
		{
			$company_birthday_data->$name = addslashes($company_birthday_data->$name);
		}
		if($broker->update($company_birthday_data,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_birthday_data",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new company_birthday_data($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_birthday_data","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$company_birthday_data = $broker->get_data(new company_birthday_data($first->id));
				$company_birthday_data->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($company_birthday_data)) as $name => $value)
				{
					$company_birthday_data->$name = addslashes($company_birthday_data->$name);	
				}
				...
				$company_birthday_data = $broker->get_data(new company_birthday_data($second->id));
				$company_birthday_data->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($company_birthday_data)) as $name => $value)
				{
					$company_birthday_data->$name = addslashes($company_birthday_data->$name);		
				}
...
$_SESSION["export_dc"] = "company_birthday_data";
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
		...
		if($all_dc_objects[$i]->garden == "1")	$all_dc_objects[$i]->garden = 1;
		else														$all_dc_objects[$i]->garden = 0;
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
		...
		if($all_dc_objects[$i]->smoking == "1")	$all_dc_objects[$i]->smoking = 1;
		else														$all_dc_objects[$i]->smoking = 0;
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
		...
		if($all_dc_objects[$i]->catering == "1")	$all_dc_objects[$i]->catering = 1;
		else														$all_dc_objects[$i]->catering = 0;
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
		...
		if($all_dc_objects[$i]->animators == "1")	$all_dc_objects[$i]->animators = 1;
		else														$all_dc_objects[$i]->animators = 0;
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
		...
		if($all_dc_objects[$i]->watching_kids == "1")	$all_dc_objects[$i]->watching_kids = 1;
		else														$all_dc_objects[$i]->watching_kids = 0;
...
$sql = $broker->execute_query("select distinct `maker` from `company_birthday_data` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Company Birthday Data</h1>
	...
	<fieldset>
		...
		<div class="item">
		<label>Ts Location</label> 
		<select name="filter_ts_location" onchange="submit()">
			<option value="all"><?php echo $ap_lang["All"]; ?></option>
 		<?php for($i=0; $i < sizeof($all_ts_location); $i++) { ?> 
			<option value="<?php echo $all_ts_location[$i]->id; ?>" <?php if($filter_ts_location == $all_ts_location[$i]->id){ ?>selected="selected"<?php } ?>><?php echo $all_ts_location[$i]->id; ?></option>
        <?php } ?>
		</select>
		</div>
		...
		<div class="item">
		<label>Garden</label> 
		<select name="filter_garden" onchange="submit()">
			<option value="all"><?php echo $ap_lang["All"]; ?></option>
 			<option value="yes" <?php if($filter_garden == "yes"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["Yes"]; ?></option>
        	<option value="no" <?php if($filter_garden == "no"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["No"]; ?></option>
		</select>
		</div>
		
		<div class="item">
		<label>Smoking</label> 
		<select name="filter_smoking" onchange="submit()">
			<option value="all"><?php echo $ap_lang["All"]; ?></option>
 			<option value="yes" <?php if($filter_smoking == "yes"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["Yes"]; ?></option>
        	<option value="no" <?php if($filter_smoking == "no"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["No"]; ?></option>
		</select>
		</div>
		
		<div class="item">
		<label>Catering</label> 
		<select name="filter_catering" onchange="submit()">
			<option value="all"><?php echo $ap_lang["All"]; ?></option>
 			<option value="yes" <?php if($filter_catering == "yes"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["Yes"]; ?></option>
        	<option value="no" <?php if($filter_catering == "no"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["No"]; ?></option>
		</select>
		</div>
		
		<div class="item">
		<label>Animators</label> 
		<select name="filter_animators" onchange="submit()">
			<option value="all"><?php echo $ap_lang["All"]; ?></option>
 			<option value="yes" <?php if($filter_animators == "yes"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["Yes"]; ?></option>
        	<option value="no" <?php if($filter_animators == "no"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["No"]; ?></option>
		</select>
		</div>
		
		<div class="item">
		<label>Watching Kids</label> 
		<select name="filter_watching_kids" onchange="submit()">
			<option value="all"><?php echo $ap_lang["All"]; ?></option>
 			<option value="yes" <?php if($filter_watching_kids == "yes"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["Yes"]; ?></option>
        	<option value="no" <?php if($filter_watching_kids == "no"){ ?>selected="selected"<?php } ?>><?php echo $ap_lang["No"]; ?></option>
		</select>
		</div>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		<th width="100">
		<?php 
		if($sort_column=="ts_location")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="ts_location";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Ts Location</a>
		</th>
		<th width="100">
		<?php 
		if($sort_column=="max_kids")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="max_kids";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Max Kids</a>
		</th>
		<th width="100">
		<?php 
		if($sort_column=="max_adults")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="max_adults";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Max Adults</a>
		</th>
		<th width="100">
		<?php 
		if($sort_column=="name")
	...
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		...
		<td><?php echo $all_dc_objects[$i]->ts_location; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->max_kids; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->max_adults; ?></td>
		<?php include_once "custom_td.php";?>
