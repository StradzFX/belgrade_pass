...
//================ SEARCH FILTER INITIALIZATION 	
	...
	if(is_numeric($search))
	{
		...
		$array_som[] = array("tranaction_id","=",$search,"OR");
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$transactions->checker = $checker;
		$transactions->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($transactions)) as $name => $value)
		{
			$transactions->$name = addslashes($transactions->$name);
		}
		if($broker->update($transactions,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"transactions",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new transactions($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"transactions","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$transactions = $broker->get_data(new transactions($first->id));
				$transactions->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($transactions)) as $name => $value)
				{
					$transactions->$name = addslashes($transactions->$name);	
				}
				$broker->update($transactions);
						
				$transactions = $broker->get_data(new transactions($second->id));
				$transactions->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($transactions)) as $name => $value)
				{
					$transactions->$name = addslashes($transactions->$name);		
				}
...
$_SESSION["export_dc"] = "transactions";
...
$sql = $broker->execute_query("select distinct `maker` from `transactions` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Transactions</h1>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		<th width="167">
		<?php 
		if($sort_column=="tranaction_id")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="tranaction_id";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Tranaction Id</a>
		</th>
		<th width="167">
		<?php 
		if($sort_column=="user")
	...
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		...
		<td><?php echo $all_dc_objects[$i]->tranaction_id; ?></td>
		<?php include_once "custom_td.php";?>
