...
//================ PHP HANDLER FOR DROPMENUPARENT CLASS training_school
...
$dc_objects = new company_location_pass_rules();
...
//================ SEARCH FILTER INITIALIZATION 	
	...
	if(is_numeric($search))
	{
		...
		$array_som[] = array("pass_per_kid","=",$search,"OR");
	}	
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$company_location_pass_rules->checker = $checker;
		$company_location_pass_rules->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($company_location_pass_rules)) as $name => $value)
		{
			$company_location_pass_rules->$name = addslashes($company_location_pass_rules->$name);
		}
		if($broker->update($company_location_pass_rules,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_location_pass_rules",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new company_location_pass_rules($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_location_pass_rules","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$company_location_pass_rules = $broker->get_data(new company_location_pass_rules($first->id));
				$company_location_pass_rules->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($company_location_pass_rules)) as $name => $value)
				{
					$company_location_pass_rules->$name = addslashes($company_location_pass_rules->$name);	
				}
				$broker->update($company_location_pass_rules);
						
				$company_location_pass_rules = $broker->get_data(new company_location_pass_rules($second->id));
				$company_location_pass_rules->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($company_location_pass_rules)) as $name => $value)
				{
					$company_location_pass_rules->$name = addslashes($company_location_pass_rules->$name);		
				}
...
$_SESSION["export_dc"] = "company_location_pass_rules";
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
	...
	$all_dc_objects[$i]->ts_location = $all_dc_objects[$i]->ts_location->street;	
...
$sql = $broker->execute_query("select distinct `maker` from `company_location_pass_rules` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Company Location Pass Rules</h1>
	...
	<fieldset>
		...
		<select name="filter_ts_location" onchange="submit()">
 		...
 		<?php for($i=0; $i < sizeof($all_ts_location); $i++) { ?> 
			<option value="<?php echo $all_ts_location[$i]->id; ?>" <?php if($filter_ts_location == $all_ts_location[$i]->id){ ?>selected="selected"<?php } ?>><?php echo $all_ts_location[$i]->street; ?></option>
        <?php } ?>
		</select>
		</div>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		<th width="71">
		<?php 
		if($sort_column=="hours_from")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="hours_from";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Hours From</a>
		</th>
		<th width="71">
		<?php 
		if($sort_column=="minutes_from")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="minutes_from";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Minutes From</a>
		</th>
		<th width="71">
		<?php 
		if($sort_column=="hours_to")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="hours_to";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Hours To</a>
		</th>
		<th width="71">
		<?php 
		if($sort_column=="minutes_to")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="minutes_to";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Minutes To</a>
		</th>
		<th width="71">
		<?php 
		if($sort_column=="pass_per_kid")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="pass_per_kid";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Pass Per Kid</a>
		</th>
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		<th width="64">
		<?php 
		...
		<td><?php echo $all_dc_objects[$i]->hours_from; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->minutes_from; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->hours_to; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->minutes_to; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->pass_per_kid; ?></td>
		<?php include_once "custom_td.php";?>
