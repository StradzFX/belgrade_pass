...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$company_location_birthday_slots->checker = $checker;
		$company_location_birthday_slots->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($company_location_birthday_slots)) as $name => $value)
		{
			$company_location_birthday_slots->$name = addslashes($company_location_birthday_slots->$name);
		}
		if($broker->update($company_location_birthday_slots,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_location_birthday_slots",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new company_location_birthday_slots($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_location_birthday_slots","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$company_location_birthday_slots = $broker->get_data(new company_location_birthday_slots($first->id));
				$company_location_birthday_slots->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($company_location_birthday_slots)) as $name => $value)
				{
					$company_location_birthday_slots->$name = addslashes($company_location_birthday_slots->$name);	
				}
				$broker->update($company_location_birthday_slots);
						
				$company_location_birthday_slots = $broker->get_data(new company_location_birthday_slots($second->id));
				$company_location_birthday_slots->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($company_location_birthday_slots)) as $name => $value)
				{
					$company_location_birthday_slots->$name = addslashes($company_location_birthday_slots->$name);		
				}
...
$_SESSION["export_dc"] = "company_location_birthday_slots";
...
$sql = $broker->execute_query("select distinct `maker` from `company_location_birthday_slots` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - CLBSlots</h1>
