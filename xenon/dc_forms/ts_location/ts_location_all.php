...
//================ SEARCH FILTER INITIALIZATION 	
	...
	if(is_numeric($search))
	{
		...
		$array_som[] = array("latitude","=",$search,"OR");
		$array_som[] = array("longitude","=",$search,"OR");
	}	
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$ts_location->checker = $checker;
		$ts_location->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($ts_location)) as $name => $value)
		{
			$ts_location->$name = addslashes($ts_location->$name);
		}
		if($broker->update($ts_location,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"ts_location",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new ts_location($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"ts_location","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$ts_location = $broker->get_data(new ts_location($first->id));
				$ts_location->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($ts_location)) as $name => $value)
				{
					$ts_location->$name = addslashes($ts_location->$name);	
				}
				$broker->update($ts_location);
						
				$ts_location = $broker->get_data(new ts_location($second->id));
				$ts_location->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($ts_location)) as $name => $value)
				{
					$ts_location->$name = addslashes($ts_location->$name);		
				}
...
$_SESSION["export_dc"] = "ts_location";
...
$sql = $broker->execute_query("select distinct `maker` from `ts_location` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - TS Location</h1>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		$sort="city";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">City</a>
		</th>
		<th width="63">
		<?php 
		if($sort_column=="part_of_city")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="part_of_city";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Part Of_city</a>
		</th>
		<th width="63">
		<?php 
		if($sort_column=="street")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="street";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Street</a>
		</th>
		<th width="63">
		<?php 
		if($sort_column=="latitude")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="latitude";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Latitude</a>
		</th>
		<th width="63">
		<?php 
		if($sort_column=="longitude")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="longitude";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Longitude</a>
		</th>
		<th width="63">
		<?php 
		...
		$sort="username";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Username</a>
		</th>
		<th width="63">
		<?php 
	...
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		...
		<td><?php echo $all_dc_objects[$i]->city; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->part_of_city; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->street; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->latitude; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->longitude; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->username; ?></td>
		<?php include_once "custom_td.php";?>
