...
//================ SEARCH FILTER INITIALIZATION 	
	...
	if(is_numeric($search))
	{
		...
		$array_som[] = array("age_from","=",$search,"OR");
		$array_som[] = array("age_to","=",$search,"OR");
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$ts_programm->checker = $checker;
		$ts_programm->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($ts_programm)) as $name => $value)
		{
			$ts_programm->$name = addslashes($ts_programm->$name);
		}
		if($broker->update($ts_programm,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"ts_programm",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new ts_programm($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"ts_programm","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$ts_programm = $broker->get_data(new ts_programm($first->id));
				$ts_programm->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($ts_programm)) as $name => $value)
				{
					$ts_programm->$name = addslashes($ts_programm->$name);	
				}
				$broker->update($ts_programm);
						
				$ts_programm = $broker->get_data(new ts_programm($second->id));
				$ts_programm->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($ts_programm)) as $name => $value)
				{
					$ts_programm->$name = addslashes($ts_programm->$name);		
				}
...
$_SESSION["export_dc"] = "ts_programm";
...
$sql = $broker->execute_query("select distinct `maker` from `ts_programm` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - TS Programm</h1>
