...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$user_kids->checker = $checker;
		$user_kids->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($user_kids)) as $name => $value)
		{
			$user_kids->$name = addslashes($user_kids->$name);
		}
		if($broker->update($user_kids,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"user_kids",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new user_kids($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"user_kids","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$user_kids = $broker->get_data(new user_kids($first->id));
				$user_kids->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($user_kids)) as $name => $value)
				{
					$user_kids->$name = addslashes($user_kids->$name);	
				}
				$broker->update($user_kids);
						
				$user_kids = $broker->get_data(new user_kids($second->id));
				$user_kids->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($user_kids)) as $name => $value)
				{
					$user_kids->$name = addslashes($user_kids->$name);		
				}
...
$sql = $broker->execute_query("select distinct `maker` from `user_kids` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - User kids</h1>
