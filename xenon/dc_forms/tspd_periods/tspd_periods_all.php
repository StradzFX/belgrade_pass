...
if($_GET)
{
	...
	$filter_tsp_day_of_week = $_GET["filter_tsp_day_of_week"];
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$tspd_periods->checker = $checker;
		$tspd_periods->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($tspd_periods)) as $name => $value)
		{
			$tspd_periods->$name = addslashes($tspd_periods->$name);
		}
		if($broker->update($tspd_periods,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"tspd_periods",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new tspd_periods($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"tspd_periods","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$tspd_periods = $broker->get_data(new tspd_periods($first->id));
				$tspd_periods->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($tspd_periods)) as $name => $value)
				{
					$tspd_periods->$name = addslashes($tspd_periods->$name);	
				}
				$broker->update($tspd_periods);
						
				$tspd_periods = $broker->get_data(new tspd_periods($second->id));
				$tspd_periods->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($tspd_periods)) as $name => $value)
				{
					$tspd_periods->$name = addslashes($tspd_periods->$name);		
				}
...
$_SESSION["export_dc"] = "tspd_periods";
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
	...
	if($all_dc_objects[$i]->tsp_day_of_week != NULL){
		
		$all_dc_objects[$i]->tsp_day_of_week = $broker->get_data(new tsp_day_of_week($all_dc_objects[$i]->tsp_day_of_week));
	$all_dc_objects[$i]->tsp_day_of_week = $all_dc_objects[$i]->tsp_day_of_week->id;	
	
	}else{
		$all_dc_objects[$i]->tsp_day_of_week = "";
	}
...
$sql = $broker->execute_query("select distinct `maker` from `tspd_periods` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - TSPD Periods</h1>
