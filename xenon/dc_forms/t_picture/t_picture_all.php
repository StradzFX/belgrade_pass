...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$t_picture->checker = $checker;
		$t_picture->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($t_picture)) as $name => $value)
		{
			$t_picture->$name = addslashes($t_picture->$name);
		}
		if($broker->update($t_picture,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"t_picture",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new t_picture($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"t_picture","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$t_picture = $broker->get_data(new t_picture($first->id));
				$t_picture->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($t_picture)) as $name => $value)
				{
					$t_picture->$name = addslashes($t_picture->$name);	
				}
				$broker->update($t_picture);
						
				$t_picture = $broker->get_data(new t_picture($second->id));
				$t_picture->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($t_picture)) as $name => $value)
				{
					$t_picture->$name = addslashes($t_picture->$name);		
				}
...
$sql = $broker->execute_query("select distinct `maker` from `t_picture` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - T Picture</h1>
...
<td><?php if($all_dc_objects[$i]->picture != ""){ ?><a id="picture" href="../pictures/t_picture/picture/<?php echo $all_dc_objects[$i]->picture; ?>"><img src="../pictures/t_picture/picture/<?php echo $all_dc_objects[$i]->picture; ?>" alt="<?php echo $all_dc_objects[$i]->picture; ?>" title="<?php echo $all_dc_objects[$i]->picture; ?>" border="0"/></a>
<?php } ?></td>
<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
