...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$ts_picture->checker = $checker;
		$ts_picture->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($ts_picture)) as $name => $value)
		{
			$ts_picture->$name = addslashes($ts_picture->$name);
		}
		if($broker->update($ts_picture,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"ts_picture",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new ts_picture($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"ts_picture","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$ts_picture = $broker->get_data(new ts_picture($first->id));
				$ts_picture->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($ts_picture)) as $name => $value)
				{
					$ts_picture->$name = addslashes($ts_picture->$name);	
				}
				$broker->update($ts_picture);
						
				$ts_picture = $broker->get_data(new ts_picture($second->id));
				$ts_picture->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($ts_picture)) as $name => $value)
				{
					$ts_picture->$name = addslashes($ts_picture->$name);		
				}
...
$sql = $broker->execute_query("select distinct `maker` from `ts_picture` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - TS Picture</h1>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		<th width="167">
		<?php 
		if($sort_column=="thumb")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="thumb";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Thumb</a>
		</th>
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		<th width="64">
		<?php 
...
<td><?php if($all_dc_objects[$i]->picture != ""){ ?><a id="picture" href="../pictures/ts_picture/picture/<?php echo $all_dc_objects[$i]->picture; ?>"><img src="../pictures/ts_picture/picture/<?php echo $all_dc_objects[$i]->picture; ?>" alt="<?php echo $all_dc_objects[$i]->picture; ?>" title="<?php echo $all_dc_objects[$i]->picture; ?>" border="0"/></a>
<?php } ?></td>
<link rel="stylesheet" href="js/fancybox/source/jquery.fancybox.css?v=2.0.4" type="text/css" media="screen" />
<script type="text/javascript" src="js/fancybox/source/jquery.fancybox.pack.js?v=2.0.4"></script>
<script>
...
<td><?php if($all_dc_objects[$i]->thumb != ""){ ?><a id="thumb" href="../pictures/ts_picture/thumb/<?php echo $all_dc_objects[$i]->thumb; ?>"><img src="../pictures/ts_picture/thumb/<?php echo $all_dc_objects[$i]->thumb; ?>" alt="<?php echo $all_dc_objects[$i]->thumb; ?>" title="<?php echo $all_dc_objects[$i]->thumb; ?>" border="0"/></a>
<?php } ?></td>
<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
