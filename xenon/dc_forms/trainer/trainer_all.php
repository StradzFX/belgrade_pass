...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$trainer->checker = $checker;
		$trainer->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($trainer)) as $name => $value)
		{
			$trainer->$name = addslashes($trainer->$name);
		}
		if($broker->update($trainer,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"trainer",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new trainer($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"trainer","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$trainer = $broker->get_data(new trainer($first->id));
				$trainer->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($trainer)) as $name => $value)
				{
					$trainer->$name = addslashes($trainer->$name);	
				}
				$broker->update($trainer);
						
				$trainer = $broker->get_data(new trainer($second->id));
				$trainer->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($trainer)) as $name => $value)
				{
					$trainer->$name = addslashes($trainer->$name);		
				}
...
$sql = $broker->execute_query("select distinct `maker` from `trainer` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Trainer</h1>
