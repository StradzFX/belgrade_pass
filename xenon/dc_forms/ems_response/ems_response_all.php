...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$ems_response->checker = $checker;
		$ems_response->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($ems_response)) as $name => $value)
		{
			$ems_response->$name = addslashes($ems_response->$name);
		}
		if($broker->update($ems_response,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"ems_response",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new ems_response($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"ems_response","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$ems_response = $broker->get_data(new ems_response($first->id));
				$ems_response->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($ems_response)) as $name => $value)
				{
					$ems_response->$name = addslashes($ems_response->$name);	
				}
				$broker->update($ems_response);
						
				$ems_response = $broker->get_data(new ems_response($second->id));
				$ems_response->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($ems_response)) as $name => $value)
				{
					$ems_response->$name = addslashes($ems_response->$name);		
				}
...
$_SESSION["export_dc"] = "ems_response";
...
$sql = $broker->execute_query("select distinct `maker` from `ems_response` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - ems_response</h1>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		<th width="500">
		<?php 
		if($sort_column=="response_date")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="response_date";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Response Date</a>
		</th>
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		<th width="64">
		<?php 
...
<td><?php $niz = explode("-",$all_dc_objects[$i]->response_date); $all_dc_objects[$i]->response_date = $niz[1]."/".$niz[2]."/".$niz[0];  echo $all_dc_objects[$i]->response_date; ?></td>
<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
