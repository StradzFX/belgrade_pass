...
if($_SESSION[ADMINMAKER]==1 && $_SESSION[ADMINCHECKER]==0 && $company_birthday_reservation->checker != "")
{
...
//=================================================================================================
	...
	if($error_message == "")
	{
		...
		if($broker->update(new company_birthday_reservation($id,$user,$user_card,$training_school,$ts_location,$birthday_date,$birthday_from_hours,$birthday_from_minutes,$birthday_to_hours,$birthday_to_minutes,$status,$comment,$number_of_kids,$number_of_adults,$company_birthday_data,$company_location_birthday_slots,$company_birthday_reservation->maker,$company_birthday_reservation->makerDate,$checker,$checkerDate,$company_birthday_reservation->pozicija,$company_birthday_reservation->jezik,$company_birthday_reservation->recordStatus,$company_birthday_reservation->modNumber+1,$company_birthday_reservation->multilang_id)) == 1)
		{
			$success_message = $ap_lang["This object was edited successfully!"];
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_birthday_reservation","edit",$_GET["id"]));
		}
		else	$error_message = $ap_lang["There was an error while editing data!"];
	}
}
$company_birthday_reservation = $broker->get_data(new company_birthday_reservation($_GET["id"]));
foreach($company_birthday_reservation as $key => $value)
	$company_birthday_reservation->$key = htmlentities($company_birthday_reservation->$key,ENT_COMPAT,"UTF-8");
...
if(isset($_POST["promote"]) && $company_birthday_reservation->checker == "")	
	$company_birthday_reservation->checker = $_SESSION[ADMINLOGGEDIN];
?>
<script src="js/wledit/wledit.js" type="text/javascript"></script>
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["Edit"]; ?> - Company birthday reservation</h1>
