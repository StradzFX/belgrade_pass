...
$dc_objects = new company_events();
$dc_objects->add_condition("recordStatus","=","O");
if($search != "")
{
//================ SEARCH FILTER INITIALIZATION 	

	$array_som[] = array("event_type","LIKE","%".$search."%","OR");
	$array_som[] = array("event_name","LIKE","%".$search."%","OR");
	$array_som[] = array("event_horus_from","LIKE","%".$search."%","OR");
	$array_som[] = array("event_hours_to","LIKE","%".$search."%","OR");
	$array_som[] = array("event_minutes_from","LIKE","%".$search."%","OR");
	$array_som[] = array("event_minutes_to","LIKE","%".$search."%","OR");
	$array_som[] = array("event_global_type","LIKE","%".$search."%","OR");
	if(is_numeric($search))
	{
		$array_som[] = array("id","=",$search,"OR");
		$array_som[] = array("training_school","=",$search,"OR");
		$array_som[] = array("event_time_from","=",$search,"OR");
		$array_som[] = array("event_time_to","=",$search,"OR");
...
if($filter_event_type != "all" && $filter_event_type != "")
	$dc_objects->add_condition("event_type","=",$filter_event_type);

if($filter_event_name != "all" && $filter_event_name != "")
	$dc_objects->add_condition("event_name","=",$filter_event_name);

if($filter_event_horus_from != "all" && $filter_event_horus_from != "")
	$dc_objects->add_condition("event_horus_from","=",$filter_event_horus_from);

if($filter_event_hours_to != "all" && $filter_event_hours_to != "")
	$dc_objects->add_condition("event_hours_to","=",$filter_event_hours_to);

if($filter_event_minutes_from != "all" && $filter_event_minutes_from != "")
	$dc_objects->add_condition("event_minutes_from","=",$filter_event_minutes_from);

if($filter_event_minutes_to != "all" && $filter_event_minutes_to != "")
	$dc_objects->add_condition("event_minutes_to","=",$filter_event_minutes_to);

if($filter_event_global_type != "all" && $filter_event_global_type != "")
	$dc_objects->add_condition("event_global_type","=",$filter_event_global_type);
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$company_events->checker = $checker;
		$company_events->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($company_events)) as $name => $value)
		{
			$company_events->$name = addslashes($company_events->$name);
		}
		if($broker->update($company_events,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_events",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new company_events($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_events","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$company_events = $broker->get_data(new company_events($first->id));
				$company_events->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($company_events)) as $name => $value)
				{
					$company_events->$name = addslashes($company_events->$name);	
				}
				...
				$company_events = $broker->get_data(new company_events($second->id));
				$company_events->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($company_events)) as $name => $value)
				{
					$company_events->$name = addslashes($company_events->$name);		
				}
...
$_SESSION["export_dc"] = "company_events";
...
$sql = $broker->execute_query("select distinct `maker` from `company_events` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Company periods</h1>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		$sort="event_date";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Event Date</a>
		</th>
		<th width="167">
		<?php 
		if($sort_column=="event_time_from")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="event_time_from";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Event Time_from</a>
		</th>
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		<th width="64">
		<?php 
...
<td><?php $niz = explode("-",$all_dc_objects[$i]->event_date); $all_dc_objects[$i]->event_date = $niz[1]."/".$niz[2]."/".$niz[0];  echo $all_dc_objects[$i]->event_date; ?></td>
		<td><?php echo $all_dc_objects[$i]->event_time_from; ?></td>
		<?php include_once "custom_td.php";?>
