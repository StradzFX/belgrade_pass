...
//================ SEARCH FILTER INITIALIZATION 	
	...
	$array_som[] = array("logo","LIKE","%".$search."%","OR");
	$array_som[] = array("map_logo","LIKE","%".$search."%","OR");
	if(is_numeric($search))
	{
		$array_som[] = array("id","=",$search,"OR");
		$array_som[] = array("popularity","=",$search,"OR");
	}	
...
if($filter_logo != "all" && $filter_logo != "")
	$dc_objects->add_condition("logo","=",$filter_logo);

if($filter_map_logo != "all" && $filter_map_logo != "")
	$dc_objects->add_condition("map_logo","=",$filter_map_logo);
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$sport_category->checker = $checker;
		$sport_category->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($sport_category)) as $name => $value)
		{
			$sport_category->$name = addslashes($sport_category->$name);
		}
		if($broker->update($sport_category,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"sport_category",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new sport_category($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"sport_category","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$sport_category = $broker->get_data(new sport_category($first->id));
				$sport_category->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($sport_category)) as $name => $value)
				{
					$sport_category->$name = addslashes($sport_category->$name);	
				}
				$broker->update($sport_category);
						
				$sport_category = $broker->get_data(new sport_category($second->id));
				$sport_category->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($sport_category)) as $name => $value)
				{
					$sport_category->$name = addslashes($sport_category->$name);		
				}
...
$_SESSION["export_dc"] = "sport_category";
...
$sql = $broker->execute_query("select distinct `maker` from `sport_category` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Category</h1>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		$sort="logo";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Logo</a>
		</th>
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		<th width="64">
		<?php 
...
<td><?php if($all_dc_objects[$i]->logo != ""){ ?><a id="logo" href="../pictures/sport_category/logo/<?php echo $all_dc_objects[$i]->logo; ?>"><img src="../pictures/sport_category/logo/<?php echo $all_dc_objects[$i]->logo; ?>" alt="<?php echo $all_dc_objects[$i]->logo; ?>" title="<?php echo $all_dc_objects[$i]->logo; ?>" border="0"/></a>
<?php } ?></td>
<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
