...
//================ PHP HANDLER FOR DROPMENUEXT CLASS purchase	
...
$all_purchase = $broker->get_all_data_condition($purchase); 
...
if($_GET)
{
	...
	$filter_purchase = $_GET["filter_purchase"];
	$filter_training_school = $_GET["filter_training_school"];
	$filter_user = $_GET["filter_user"];
	$filter_pass_type = $_GET["filter_pass_type"];
...
//================ SEARCH FILTER INITIALIZATION 	
	...
	if(is_numeric($search))
	{
		...
		$array_som[] = array("taken_passes","=",$search,"OR");
...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$accepted_passes->checker = $checker;
		$accepted_passes->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($accepted_passes)) as $name => $value)
		{
			$accepted_passes->$name = addslashes($accepted_passes->$name);
		}
		if($broker->update($accepted_passes,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"accepted_passes",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new accepted_passes($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"accepted_passes","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$accepted_passes = $broker->get_data(new accepted_passes($first->id));
				$accepted_passes->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($accepted_passes)) as $name => $value)
				{
					$accepted_passes->$name = addslashes($accepted_passes->$name);	
				}
				$broker->update($accepted_passes);
						
				$accepted_passes = $broker->get_data(new accepted_passes($second->id));
				$accepted_passes->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($accepted_passes)) as $name => $value)
				{
					$accepted_passes->$name = addslashes($accepted_passes->$name);		
				}
...
$_SESSION["export_dc"] = "accepted_passes";
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
	...
	if($all_dc_objects[$i]->purchase != NULL){
		
		$all_dc_objects[$i]->purchase = $broker->get_data(new purchase($all_dc_objects[$i]->purchase));
	$all_dc_objects[$i]->purchase = $all_dc_objects[$i]->purchase->id;	
	
	}else{
		$all_dc_objects[$i]->purchase = "";
	}
...
$sql = $broker->execute_query("select distinct `maker` from `accepted_passes` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Accepted Passes</h1>
	...
	<fieldset>
		...
		<select name="filter_purchase" onchange="submit()">
			<option value="all"><?php echo $ap_lang["All"]; ?></option>
 		<?php for($i=0; $i < sizeof($all_purchase); $i++) { ?> 
			<option value="<?php echo $all_purchase[$i]->id; ?>" <?php if($filter_purchase == $all_purchase[$i]->id){ ?>selected="selected"<?php } ?>><?php echo $all_purchase[$i]->id; ?></option>
        <?php } ?>
		</select>
		</div>
		...
		<select name="filter_pass_type" onchange="submit()">
			<option value="all"><?php echo $ap_lang["All"]; ?></option>
			<option value="regular" <?php if($filter_pass_type == "regular"){ ?>selected="selected"<?php } ?>>regular</option>
			<option value="birthday" <?php if($filter_pass_type == "birthday"){ ?>selected="selected"<?php } ?>>birthday</option>   
		</select>
		</div>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		<th width="83">
		<?php 
		if($sort_column=="purchase")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="purchase";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Purchase</a>
		</th>
		<th width="83">
		<?php 
		if($sort_column=="taken_passes")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="taken_passes";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Taken Passes</a>
		</th>
		<th width="83">
		<?php 
		...
		<th width="83">
		<?php 
		if($sort_column=="number_of_kids")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="number_of_kids";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Number Of_kids</a>
		</th>
		<th width="83">
		<?php 
		if($sort_column=="user")
	...
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		...
		<td><?php echo $all_dc_objects[$i]->purchase; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->taken_passes; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->training_school; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->number_of_kids; ?></td>
		<?php include_once "custom_td.php";?>
