...
//================ PROMOTE ITEM PROCESSING
	if($_GET["promote"] != "")
	{
		...
		$company_discount_rules->checker = $checker;
		$company_discount_rules->checkerDate = $checker_date;
		foreach(get_class_vars(get_class($company_discount_rules)) as $name => $value)
		{
			$company_discount_rules->$name = addslashes($company_discount_rules->$name);
		}
		if($broker->update($company_discount_rules,true) >= 1)
			$dc_object = $broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_discount_rules",$action,$_GET["id"]),false,true,false);
	}
//================ DELETE ITEM PROCESSING
	if($_GET["delete"])
	{
		$dc_object = $broker->get_data(new company_discount_rules($_GET["delete"]));
		...
		if($broker->update($dc_object) >= 1)	
		{
			...
			$broker->insert(new xenon_dataaudittrail(0,$_SERVER["REMOTE_ADDR"],date("Y-m-d H:i:s"),$_SESSION[ADMINLOGGEDIN],"company_discount_rules","delete",$_GET["delete"]));
		}
	...
	if($_GET["pos"])
	{
		...
		for($i = 0 ; $i < sizeof($all_dc_objects) ; $i++)
		{
			if($all_dc_objects[$i]->id == $_GET["id"])
			{
				...
				$company_discount_rules = $broker->get_data(new company_discount_rules($first->id));
				$company_discount_rules->pozicija = $first->pozicija;
				foreach(get_class_vars(get_class($company_discount_rules)) as $name => $value)
				{
					$company_discount_rules->$name = addslashes($company_discount_rules->$name);	
				}
				...
				$company_discount_rules = $broker->get_data(new company_discount_rules($second->id));
				$company_discount_rules->pozicija = $second->pozicija;
				foreach(get_class_vars(get_class($company_discount_rules)) as $name => $value)
				{
					$company_discount_rules->$name = addslashes($company_discount_rules->$name);		
				}
...
$_SESSION["export_dc"] = "company_discount_rules";
...
for($i=0; $i<sizeof($all_dc_objects); $i++)
{
	...
	$all_dc_objects[$i]->training_school = $all_dc_objects[$i]->training_school->id;	
...
$sql = $broker->execute_query("select distinct `maker` from `company_discount_rules` where `recordStatus` = 'O'");
$makers = array();
...
<div id="container">
	<div id="top">
	<h1><?php echo $ap_lang["List of all entries for object"]; ?> - Company discount rules</h1>
	...
	<fieldset>
		...
		<select name="filter_training_school" onchange="submit()">
 		...
 		<?php for($i=0; $i < sizeof($all_training_school); $i++) { ?> 
			<option value="<?php echo $all_training_school[$i]->id; ?>" <?php if($filter_training_school == $all_training_school[$i]->id){ ?>selected="selected"<?php } ?>><?php echo $all_training_school[$i]->id; ?></option>
        <?php } ?>
		</select>
		</div>
	...
	<table width="757" border="0" cellpadding="0" cellspacing="0">
		...
		<th width="83">
		<?php 
		if($sort_column=="day_from")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="day_from";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Day From</a>
		</th>
		<th width="83">
		<?php 
		if($sort_column=="day_to")
			if($sort_direction == "asc")	$sort_direction = "desc";
			else							$sort_direction = "asc";
		$sort="day_to";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Day To</a>
		</th>
		<th width="83">
		<?php 
		...
		$sort="discount";
        ?>
		<a href="<?php echo url_link("id&pos&promote&delete&sort_column=".$sort."&sort_direction=".$sort_direction); ?>">Discount</a>
		</th>
	<?php if(unserialize($_SESSION[ADMINUSER])->see_other_data != 0 || !property_exists("xenon_user","see_other_data")){ ?>
		<th width="64">
		<?php 
		...
		<td><?php echo $all_dc_objects[$i]->day_from; ?></td>
		<?php include_once "custom_td.php";?>
		
		<td><?php echo $all_dc_objects[$i]->day_to; ?></td>
		<?php include_once "custom_td.php";?>
		...
		<td><?php echo $all_dc_objects[$i]->discount; ?></td>
		<?php include_once "custom_td.php";?>
