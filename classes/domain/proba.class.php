...
class proba implements base_domain_object
{
	public $id = 0;
	public $prezime = "";
	public $ime = "";
	...
	private $varchar = array("prezime","ime","opis","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","prezime","ime","godine","opis");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $prezime = "", $ime = "", $godine = 0, $opis = "", $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		if(is_numeric($id)){ $this->id = $id;}else{$this->id = 0;}
		$this->prezime = $prezime;
		$this->ime = $ime;
		if(is_numeric($godine)){ $this->godine = $godine;}else{$this->godine = 0;}
		$this->opis = $opis;
		$this->maker = $maker;
	...
	{	
		return "`id`,`prezime`,`ime`,`godine`,`opis`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,'{$this->prezime}','{$this->ime}',{$this->godine},'{$this->opis}','{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new proba($row['id'],$row['prezime'],$row['ime'],$row['godine'],$row['opis'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	{
		return "`prezime` = '{$this->prezime}',`ime` = '{$this->ime}',`godine` = {$this->godine},`opis` = '{$this->opis}',`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
