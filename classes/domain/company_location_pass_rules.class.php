...
class company_location_pass_rules implements base_domain_object
{
	public $id = 0;
	public $training_school = 0;
	public $ts_location = 0;
	public $hours_from = 0;
	public $minutes_from = 0;
	public $hours_to = 0;
	public $minutes_to = 0;
	public $pass_per_kid = 0;
	public $maker = "";
	public $makerDate = "";
	public $checker = "";
	public $checkerDate = "";
	...
	public $domain_fields_array = array("id","training_school","ts_location","hours_from","minutes_from","hours_to","minutes_to","pass_per_kid");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $training_school = 0, $ts_location = 0, $hours_from = 0, $minutes_from = 0, $hours_to = 0, $minutes_to = 0, $pass_per_kid = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		...
		if(is_numeric($minutes_from)){ $this->minutes_from = $minutes_from;}else{$this->minutes_from = 0;}
		if(is_numeric($hours_to)){ $this->hours_to = $hours_to;}else{$this->hours_to = 0;}
		if(is_numeric($minutes_to)){ $this->minutes_to = $minutes_to;}else{$this->minutes_to = 0;}
		if(is_numeric($pass_per_kid)){ $this->pass_per_kid = $pass_per_kid;}else{$this->pass_per_kid = 0;}
		$this->maker = $maker;
	...
	{	
		return "`id`,`training_school`,`ts_location`,`hours_from`,`minutes_from`,`hours_to`,`minutes_to`,`pass_per_kid`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,{$this->training_school},{$this->ts_location},{$this->hours_from},{$this->minutes_from},{$this->hours_to},{$this->minutes_to},{$this->pass_per_kid},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new company_location_pass_rules($row['id'],$row['training_school'],$row['ts_location'],$row['hours_from'],$row['minutes_from'],$row['hours_to'],$row['minutes_to'],$row['pass_per_kid'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	{
		return "`training_school` = {$this->training_school},`ts_location` = {$this->ts_location},`hours_from` = {$this->hours_from},`minutes_from` = {$this->minutes_from},`hours_to` = {$this->hours_to},`minutes_to` = {$this->minutes_to},`pass_per_kid` = {$this->pass_per_kid},`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
