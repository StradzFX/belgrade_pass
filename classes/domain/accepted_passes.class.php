...
class accepted_passes implements base_domain_object
{
	public $id = 0;
	public $user_card = 0;
	public $purchase = 0;
	public $taken_passes = 0;
	public $training_school = 0;
	public $number_of_kids = 0;
	public $user = 0;
	public $pay_to_company = 0;
	public $pay_to_us = 0;
	...
	private $varchar = array("pass_type","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","user_card","purchase","taken_passes","training_school","number_of_kids","user","pay_to_company","pay_to_us","company_location","pass_type","reservation_id");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $user_card = 0, $purchase = 0, $taken_passes = 0, $training_school = 0, $number_of_kids = 0, $user = 0, $pay_to_company = 0, $pay_to_us = 0, $company_location = 0, $pass_type = "", $reservation_id = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		if(is_numeric($id)){ $this->id = $id;}else{$this->id = 0;}
		if(is_numeric($user_card)){ $this->user_card = $user_card;}else{$this->user_card = 0;}
		if(is_numeric($purchase)){ $this->purchase = $purchase;}else{$this->purchase = 0;}
		if(is_numeric($taken_passes)){ $this->taken_passes = $taken_passes;}else{$this->taken_passes = 0;}
		...
		if(is_numeric($pay_to_company)){ $this->pay_to_company = $pay_to_company;}else{$this->pay_to_company = 0;}
		if(is_numeric($pay_to_us)){ $this->pay_to_us = $pay_to_us;}else{$this->pay_to_us = 0;}
		if(is_numeric($company_location)){ $this->company_location = $company_location;}else{$this->company_location = 0;}
		$this->pass_type = $pass_type;
		if(is_numeric($reservation_id)){ $this->reservation_id = $reservation_id;}else{$this->reservation_id = 0;}
		$this->maker = $maker;
	...
	{	
		return "`id`,`user_card`,`purchase`,`taken_passes`,`training_school`,`number_of_kids`,`user`,`pay_to_company`,`pay_to_us`,`company_location`,`pass_type`,`reservation_id`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,{$this->user_card},{$this->purchase},{$this->taken_passes},{$this->training_school},{$this->number_of_kids},{$this->user},{$this->pay_to_company},{$this->pay_to_us},{$this->company_location},'{$this->pass_type}',{$this->reservation_id},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new accepted_passes($row['id'],$row['user_card'],$row['purchase'],$row['taken_passes'],$row['training_school'],$row['number_of_kids'],$row['user'],$row['pay_to_company'],$row['pay_to_us'],$row['company_location'],$row['pass_type'],$row['reservation_id'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	{
		return "`user_card` = {$this->user_card},`purchase` = {$this->purchase},`taken_passes` = {$this->taken_passes},`training_school` = {$this->training_school},`number_of_kids` = {$this->number_of_kids},`user` = {$this->user},`pay_to_company` = {$this->pay_to_company},`pay_to_us` = {$this->pay_to_us},`company_location` = {$this->company_location},`pass_type` = '{$this->pass_type}',`reservation_id` = {$this->reservation_id},`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
