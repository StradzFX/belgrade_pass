...
class admin_links implements base_domain_object
{
	public $id = 0;
	public $name = "";
	public $link = "";
	...
	private $varchar = array("name","link","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $name = "", $link = "", $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		$this->id = $id;
		$this->name = $name;
		$this->link = $link;
	...
	$this->display_elements_formats["link"] = array("database_type"=>"VARCHAR","html_type"=>"INPUT");
	...
	public function get_attribute_list()
	{
		...
		return "`id`,`name`,`link`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,'{$this->name}','{$this->link}','{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new admin_links($row['id'],$row['name'],$row['link'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	public function get_update_values()
	{
		...
		return "`name` = '{$this->name}',`link` = '{$this->link}',`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
}
?>