...
class transactions implements base_domain_object
{
	public $id = 0;
	public $transaction_type = "";
	public $tranaction_id = 0;
	public $user = 0;
	public $maker = "";
	public $makerDate = "";
	public $checker = "";
	...
	private $varchar = array("transaction_type","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","transaction_type","tranaction_id","user");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $transaction_type = "", $tranaction_id = 0, $user = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		if(is_numeric($id)){ $this->id = $id;}else{$this->id = 0;}
		$this->transaction_type = $transaction_type;
		if(is_numeric($tranaction_id)){ $this->tranaction_id = $tranaction_id;}else{$this->tranaction_id = 0;}
	...
	{	
		return "`id`,`transaction_type`,`tranaction_id`,`user`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,'{$this->transaction_type}',{$this->tranaction_id},{$this->user},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new transactions($row['id'],$row['transaction_type'],$row['tranaction_id'],$row['user'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
