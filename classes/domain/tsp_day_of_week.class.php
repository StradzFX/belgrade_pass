...
class tsp_day_of_week implements base_domain_object
{
	public $id = 0;
	public $ts_programm = 0;
	public $name = "";
	...
	public $domain_fields_array = array("id","ts_programm","name","day_of_week","trainer");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $ts_programm = 0, $name = "", $day_of_week = 0, $trainer = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		if(is_numeric($id)){ $this->id = $id;}else{$this->id = 0;}
		if(is_numeric($ts_programm)){ $this->ts_programm = $ts_programm;}else{$this->ts_programm = 0;}
		$this->name = $name;
		if(is_numeric($day_of_week)){ $this->day_of_week = $day_of_week;}else{$this->day_of_week = 0;}
	...
	{	
		return "`id`,`ts_programm`,`name`,`day_of_week`,`trainer`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,{$this->ts_programm},'{$this->name}',{$this->day_of_week},{$this->trainer},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new tsp_day_of_week($row['id'],$row['ts_programm'],$row['name'],$row['day_of_week'],$row['trainer'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
