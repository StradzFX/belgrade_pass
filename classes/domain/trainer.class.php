...
class trainer implements base_domain_object
{
	public $id = 0;
	public $first_name = "";
	public $last_name = "";
	...
	private $varchar = array("first_name","last_name","photo","short_description","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","first_name","last_name","photo","short_description","sport_category");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $first_name = "", $last_name = "", $photo = "", $short_description = "", $sport_category = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
	...
	{	
		return "`id`,`first_name`,`last_name`,`photo`,`short_description`,`sport_category`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,'{$this->first_name}','{$this->last_name}','{$this->photo}','{$this->short_description}',{$this->sport_category},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new trainer($row['id'],$row['first_name'],$row['last_name'],$row['photo'],$row['short_description'],$row['sport_category'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
