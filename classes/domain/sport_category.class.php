...
class sport_category implements base_domain_object
{
	public $id = 0;
	public $name = "";
	public $logo = "";
	...
	private $varchar = array("name","logo","map_logo","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","name","logo","popularity","map_logo");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $name = "", $logo = "", $popularity = 0, $map_logo = "", $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		if(is_numeric($id)){ $this->id = $id;}else{$this->id = 0;}
		$this->name = $name;
		$this->logo = $logo;
		if(is_numeric($popularity)){ $this->popularity = $popularity;}else{$this->popularity = 0;}
		$this->map_logo = $map_logo;
		$this->maker = $maker;
	...
	{	
		return "`id`,`name`,`logo`,`popularity`,`map_logo`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,'{$this->name}','{$this->logo}',{$this->popularity},'{$this->map_logo}','{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new sport_category($row['id'],$row['name'],$row['logo'],$row['popularity'],$row['map_logo'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
