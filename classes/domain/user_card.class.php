...
class user_card implements base_domain_object
{
	public $id = 0;
	public $card_number = "";
	...
	public $post_postal = "";
	public $partner_id = "";
	public $customer_received = 0;
	public $parent_first_name = "";
	public $number_of_kids = 0;
	public $city = "";
	public $phone = "";
	public $email = "";
	...
	private $varchar = array("card_number","child_birthdate","card_password","delivery_method","post_street","post_city","post_postal","partner_id","parent_first_name","city","phone","email","parent_last_name","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","card_number","child_birthdate","user","card_password","delivery_method","post_street","post_city","post_postal","partner_id","customer_received","parent_first_name","number_of_kids","city","phone","email","company_location","parent_last_name");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $card_number = "", $child_birthdate = "", $user = 0, $card_password = "", $delivery_method = "", $post_street = "", $post_city = "", $post_postal = "", $partner_id = "", $customer_received = 0, $parent_first_name = "", $number_of_kids = 0, $city = "", $phone = "", $email = "", $company_location = 0, $parent_last_name = "", $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		...
		$this->post_postal = $post_postal;
		$this->partner_id = $partner_id;
		if(is_numeric($customer_received)){ $this->customer_received = $customer_received;}else{$this->customer_received = 0;}
		$this->parent_first_name = $parent_first_name;
		if(is_numeric($number_of_kids)){ $this->number_of_kids = $number_of_kids;}else{$this->number_of_kids = 0;}
		$this->city = $city;
		$this->phone = $phone;
		$this->email = $email;
	...
	{	
		return "`id`,`card_number`,`child_birthdate`,`user`,`card_password`,`delivery_method`,`post_street`,`post_city`,`post_postal`,`partner_id`,`customer_received`,`parent_first_name`,`number_of_kids`,`city`,`phone`,`email`,`company_location`,`parent_last_name`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,'{$this->card_number}','{$this->child_birthdate}',{$this->user},'{$this->card_password}','{$this->delivery_method}','{$this->post_street}','{$this->post_city}','{$this->post_postal}','{$this->partner_id}',{$this->customer_received},'{$this->parent_first_name}',{$this->number_of_kids},'{$this->city}','{$this->phone}','{$this->email}',{$this->company_location},'{$this->parent_last_name}','{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new user_card($row['id'],$row['card_number'],$row['child_birthdate'],$row['user'],$row['card_password'],$row['delivery_method'],$row['post_street'],$row['post_city'],$row['post_postal'],$row['partner_id'],$row['customer_received'],$row['parent_first_name'],$row['number_of_kids'],$row['city'],$row['phone'],$row['email'],$row['company_location'],$row['parent_last_name'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	{
		return "`card_number` = '{$this->card_number}',`child_birthdate` = '{$this->child_birthdate}',`user` = {$this->user},`card_password` = '{$this->card_password}',`delivery_method` = '{$this->delivery_method}',`post_street` = '{$this->post_street}',`post_city` = '{$this->post_city}',`post_postal` = '{$this->post_postal}',`partner_id` = '{$this->partner_id}',`customer_received` = {$this->customer_received},`parent_first_name` = '{$this->parent_first_name}',`number_of_kids` = {$this->number_of_kids},`city` = '{$this->city}',`phone` = '{$this->phone}',`email` = '{$this->email}',`company_location` = {$this->company_location},`parent_last_name` = '{$this->parent_last_name}',`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
