...
class card_package implements base_domain_object
{
	public $id = 0;
	public $name = "";
	public $picture = "";
	...
	private $varchar = array("name","picture","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","name","picture","price","to_company","to_us","duration_days","number_of_passes","best_value");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $name = "", $picture = "", $price = 0, $to_company = 0, $to_us = 0, $duration_days = 0, $number_of_passes = 0, $best_value = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		if(is_numeric($id)){ $this->id = $id;}else{$this->id = 0;}
		$this->name = $name;
		$this->picture = $picture;
		if(is_numeric($price)){ $this->price = $price;}else{$this->price = 0;}
		if(is_numeric($to_company)){ $this->to_company = $to_company;}else{$this->to_company = 0;}
		if(is_numeric($to_us)){ $this->to_us = $to_us;}else{$this->to_us = 0;}
		if(is_numeric($duration_days)){ $this->duration_days = $duration_days;}else{$this->duration_days = 0;}
		if(is_numeric($number_of_passes)){ $this->number_of_passes = $number_of_passes;}else{$this->number_of_passes = 0;}
		if(is_numeric($best_value)){ $this->best_value = $best_value;}else{$this->best_value = 0;}
		$this->maker = $maker;
	...
	{	
		return "`id`,`name`,`picture`,`price`,`to_company`,`to_us`,`duration_days`,`number_of_passes`,`best_value`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,'{$this->name}','{$this->picture}',{$this->price},{$this->to_company},{$this->to_us},{$this->duration_days},{$this->number_of_passes},{$this->best_value},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new card_package($row['id'],$row['name'],$row['picture'],$row['price'],$row['to_company'],$row['to_us'],$row['duration_days'],$row['number_of_passes'],$row['best_value'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
