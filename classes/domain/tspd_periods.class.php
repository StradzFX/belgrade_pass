...
class tspd_periods implements base_domain_object
{
	public $id = 0;
	public $tsp_day_of_week = 0;
	public $time_from = 0;
	public $time_to = 0;
	public $price = 0;
	public $ccy = "";
	...
	private $varchar = array("ccy","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","tsp_day_of_week","time_from","time_to","price","ccy","trainer");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $tsp_day_of_week = 0, $time_from = 0, $time_to = 0, $price = 0, $ccy = "", $trainer = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		if(is_numeric($id)){ $this->id = $id;}else{$this->id = 0;}
		if(is_numeric($tsp_day_of_week)){ $this->tsp_day_of_week = $tsp_day_of_week;}else{$this->tsp_day_of_week = 0;}
		if(is_numeric($time_from)){ $this->time_from = $time_from;}else{$this->time_from = 0;}
		if(is_numeric($time_to)){ $this->time_to = $time_to;}else{$this->time_to = 0;}
		if(is_numeric($price)){ $this->price = $price;}else{$this->price = 0;}
		$this->ccy = $ccy;
		if(is_numeric($trainer)){ $this->trainer = $trainer;}else{$this->trainer = 0;}
		$this->maker = $maker;
	...
	{	
		return "`id`,`tsp_day_of_week`,`time_from`,`time_to`,`price`,`ccy`,`trainer`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,{$this->tsp_day_of_week},{$this->time_from},{$this->time_to},{$this->price},'{$this->ccy}',{$this->trainer},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new tspd_periods($row['id'],$row['tsp_day_of_week'],$row['time_from'],$row['time_to'],$row['price'],$row['ccy'],$row['trainer'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
