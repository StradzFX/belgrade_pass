...
class company_discount_rules implements base_domain_object
{
	public $id = 0;
	...
	{	
		return "`id`,`training_school`,`day_from`,`day_to`,`hours_from`,`hours_to`,`discount`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}
	...
	{
		$new_object = new company_discount_rules($row['id'],$row['training_school'],$row['day_from'],$row['day_to'],$row['hours_from'],$row['hours_to'],$row['discount'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
