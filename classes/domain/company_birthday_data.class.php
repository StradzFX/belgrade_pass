...
class company_birthday_data implements base_domain_object
{
	public $id = 0;
	...
	private $varchar = array("name","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","training_school","ts_location","garden","smoking","max_kids","max_adults","catering","animators","watching_kids","name");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $training_school = 0, $ts_location = 0, $garden = 0, $smoking = 0, $max_kids = 0, $max_adults = 0, $catering = 0, $animators = 0, $watching_kids = 0, $name = "", $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		...
		if(is_numeric($garden)){ $this->garden = $garden;}else{$this->garden = 0;}
		if(is_numeric($smoking)){ $this->smoking = $smoking;}else{$this->smoking = 0;}
		if(is_numeric($max_kids)){ $this->max_kids = $max_kids;}else{$this->max_kids = 0;}
		if(is_numeric($max_adults)){ $this->max_adults = $max_adults;}else{$this->max_adults = 0;}
		if(is_numeric($catering)){ $this->catering = $catering;}else{$this->catering = 0;}
		if(is_numeric($animators)){ $this->animators = $animators;}else{$this->animators = 0;}
		if(is_numeric($watching_kids)){ $this->watching_kids = $watching_kids;}else{$this->watching_kids = 0;}
		$this->name = $name;
		$this->maker = $maker;
	...
	{	
		return "`id`,`training_school`,`ts_location`,`garden`,`smoking`,`max_kids`,`max_adults`,`catering`,`animators`,`watching_kids`,`name`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,{$this->training_school},{$this->ts_location},{$this->garden},{$this->smoking},{$this->max_kids},{$this->max_adults},{$this->catering},{$this->animators},{$this->watching_kids},'{$this->name}','{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new company_birthday_data($row['id'],$row['training_school'],$row['ts_location'],$row['garden'],$row['smoking'],$row['max_kids'],$row['max_adults'],$row['catering'],$row['animators'],$row['watching_kids'],$row['name'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	{
		return "`training_school` = {$this->training_school},`ts_location` = {$this->ts_location},`garden` = {$this->garden},`smoking` = {$this->smoking},`max_kids` = {$this->max_kids},`max_adults` = {$this->max_adults},`catering` = {$this->catering},`animators` = {$this->animators},`watching_kids` = {$this->watching_kids},`name` = '{$this->name}',`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
