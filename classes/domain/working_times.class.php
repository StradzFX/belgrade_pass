...
class working_times implements base_domain_object
{
	public $id = 0;
	public $training_school = 0;
	public $day_of_week = "";
	public $not_working = 0;
	public $working_from_hours = 0;
	public $working_from_minutes = 0;
	public $working_to_hours = 0;
	public $working_to_minutes = 0;
	...
	private $varchar = array("day_of_week","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","training_school","day_of_week","not_working","working_from_hours","working_from_minutes","working_to_hours","working_to_minutes","ts_location");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $training_school = 0, $day_of_week = "", $not_working = 0, $working_from_hours = 0, $working_from_minutes = 0, $working_to_hours = 0, $working_to_minutes = 0, $ts_location = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		...
		if(is_numeric($not_working)){ $this->not_working = $not_working;}else{$this->not_working = 0;}
		if(is_numeric($working_from_hours)){ $this->working_from_hours = $working_from_hours;}else{$this->working_from_hours = 0;}
		if(is_numeric($working_from_minutes)){ $this->working_from_minutes = $working_from_minutes;}else{$this->working_from_minutes = 0;}
		if(is_numeric($working_to_hours)){ $this->working_to_hours = $working_to_hours;}else{$this->working_to_hours = 0;}
		if(is_numeric($working_to_minutes)){ $this->working_to_minutes = $working_to_minutes;}else{$this->working_to_minutes = 0;}
	...
	{	
		return "`id`,`training_school`,`day_of_week`,`not_working`,`working_from_hours`,`working_from_minutes`,`working_to_hours`,`working_to_minutes`,`ts_location`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,{$this->training_school},'{$this->day_of_week}',{$this->not_working},{$this->working_from_hours},{$this->working_from_minutes},{$this->working_to_hours},{$this->working_to_minutes},{$this->ts_location},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new working_times($row['id'],$row['training_school'],$row['day_of_week'],$row['not_working'],$row['working_from_hours'],$row['working_from_minutes'],$row['working_to_hours'],$row['working_to_minutes'],$row['ts_location'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	{
		return "`training_school` = {$this->training_school},`day_of_week` = '{$this->day_of_week}',`not_working` = {$this->not_working},`working_from_hours` = {$this->working_from_hours},`working_from_minutes` = {$this->working_from_minutes},`working_to_hours` = {$this->working_to_hours},`working_to_minutes` = {$this->working_to_minutes},`ts_location` = {$this->ts_location},`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
