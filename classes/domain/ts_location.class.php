...
class ts_location implements base_domain_object
{
	public $id = 0;
	...
	private $varchar = array("city","part_of_city","street","username","password","email","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","training_school","city","part_of_city","street","latitude","longitude","username","password","email");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $training_school = 0, $city = "", $part_of_city = "", $street = "", $latitude = 0, $longitude = 0, $username = "", $password = "", $email = "", $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		...
		$this->city = $city;
		$this->part_of_city = $part_of_city;
		$this->street = $street;
		if(is_numeric($latitude)){ $this->latitude = $latitude;}else{$this->latitude = 0;}
		if(is_numeric($longitude)){ $this->longitude = $longitude;}else{$this->longitude = 0;}
	...
	{	
		return "`id`,`training_school`,`city`,`part_of_city`,`street`,`latitude`,`longitude`,`username`,`password`,`email`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,{$this->training_school},'{$this->city}','{$this->part_of_city}','{$this->street}',{$this->latitude},{$this->longitude},'{$this->username}','{$this->password}','{$this->email}','{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new ts_location($row['id'],$row['training_school'],$row['city'],$row['part_of_city'],$row['street'],$row['latitude'],$row['longitude'],$row['username'],$row['password'],$row['email'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	{
		return "`training_school` = {$this->training_school},`city` = '{$this->city}',`part_of_city` = '{$this->part_of_city}',`street` = '{$this->street}',`latitude` = {$this->latitude},`longitude` = {$this->longitude},`username` = '{$this->username}',`password` = '{$this->password}',`email` = '{$this->email}',`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
