...
class card_numbers implements base_domain_object
{
	public $id = 0;
	public $card_number = "";
	...
	public $card_taken = 0;
	public $user = 0;
	public $card_reserved = 0;
	public $company_card = 0;
	public $card_number_int = 0;
	public $picture = "";
	public $company_location = 0;
	public $internal_reservation = 0;
	public $maker = "";
	public $makerDate = "";
	public $checker = "";
	public $checkerDate = "";
	...
	private $varchar = array("card_number","card_password","picture","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","card_number","card_password","card_taken","user","card_reserved","company_card","card_number_int","picture","company_location","internal_reservation");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $card_number = "", $card_password = "", $card_taken = 0, $user = 0, $card_reserved = 0, $company_card = 0, $card_number_int = 0, $picture = "", $company_location = 0, $internal_reservation = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		if(is_numeric($id)){ $this->id = $id;}else{$this->id = 0;}
		$this->card_number = $card_number;
		$this->card_password = $card_password;
		if(is_numeric($card_taken)){ $this->card_taken = $card_taken;}else{$this->card_taken = 0;}
		if(is_numeric($user)){ $this->user = $user;}else{$this->user = 0;}
		if(is_numeric($card_reserved)){ $this->card_reserved = $card_reserved;}else{$this->card_reserved = 0;}
		if(is_numeric($company_card)){ $this->company_card = $company_card;}else{$this->company_card = 0;}
		if(is_numeric($card_number_int)){ $this->card_number_int = $card_number_int;}else{$this->card_number_int = 0;}
		$this->picture = $picture;
		if(is_numeric($company_location)){ $this->company_location = $company_location;}else{$this->company_location = 0;}
		if(is_numeric($internal_reservation)){ $this->internal_reservation = $internal_reservation;}else{$this->internal_reservation = 0;}
		$this->maker = $maker;
	...
	{	
		return "`id`,`card_number`,`card_password`,`card_taken`,`user`,`card_reserved`,`company_card`,`card_number_int`,`picture`,`company_location`,`internal_reservation`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,'{$this->card_number}','{$this->card_password}',{$this->card_taken},{$this->user},{$this->card_reserved},{$this->company_card},{$this->card_number_int},'{$this->picture}',{$this->company_location},{$this->internal_reservation},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new card_numbers($row['id'],$row['card_number'],$row['card_password'],$row['card_taken'],$row['user'],$row['card_reserved'],$row['company_card'],$row['card_number_int'],$row['picture'],$row['company_location'],$row['internal_reservation'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	{
		return "`card_number` = '{$this->card_number}',`card_password` = '{$this->card_password}',`card_taken` = {$this->card_taken},`user` = {$this->user},`card_reserved` = {$this->card_reserved},`company_card` = {$this->company_card},`card_number_int` = {$this->card_number_int},`picture` = '{$this->picture}',`company_location` = {$this->company_location},`internal_reservation` = {$this->internal_reservation},`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
