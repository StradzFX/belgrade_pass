...
class user_kids implements base_domain_object
{
	public $id = 0;
	public $user = 0;
	public $name = "";
	...
	private $varchar = array("name","date_of_birth","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","user","name","date_of_birth","user_card");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $user = 0, $name = "", $date_of_birth = "", $user_card = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
	...
	{	
		return "`id`,`user`,`name`,`date_of_birth`,`user_card`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,{$this->user},'{$this->name}','{$this->date_of_birth}',{$this->user_card},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new user_kids($row['id'],$row['user'],$row['name'],$row['date_of_birth'],$row['user_card'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
