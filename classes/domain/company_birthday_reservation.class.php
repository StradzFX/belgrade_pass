...
class company_birthday_reservation implements base_domain_object
{
	public $id = 0;
	public $user = 0;
	public $user_card = 0;
	...
	public $birthday_from_hours = 0;
	public $birthday_from_minutes = 0;
	public $birthday_to_hours = 0;
	public $birthday_to_minutes = 0;
	public $status = "";
	public $comment = "";
	...
	private $varchar = array("birthday_date","status","comment","number_of_kids","number_of_adults","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","user","user_card","training_school","ts_location","birthday_date","birthday_from_hours","birthday_from_minutes","birthday_to_hours","birthday_to_minutes","status","comment","number_of_kids","number_of_adults","company_birthday_data","company_location_birthday_slots");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $user = 0, $user_card = 0, $training_school = 0, $ts_location = 0, $birthday_date = "", $birthday_from_hours = 0, $birthday_from_minutes = 0, $birthday_to_hours = 0, $birthday_to_minutes = 0, $status = "", $comment = "", $number_of_kids = "", $number_of_adults = "", $company_birthday_data = 0, $company_location_birthday_slots = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		...
		if(is_numeric($birthday_from_hours)){ $this->birthday_from_hours = $birthday_from_hours;}else{$this->birthday_from_hours = 0;}
		if(is_numeric($birthday_from_minutes)){ $this->birthday_from_minutes = $birthday_from_minutes;}else{$this->birthday_from_minutes = 0;}
		if(is_numeric($birthday_to_hours)){ $this->birthday_to_hours = $birthday_to_hours;}else{$this->birthday_to_hours = 0;}
		if(is_numeric($birthday_to_minutes)){ $this->birthday_to_minutes = $birthday_to_minutes;}else{$this->birthday_to_minutes = 0;}
		$this->status = $status;
	...
	{	
		return "`id`,`user`,`user_card`,`training_school`,`ts_location`,`birthday_date`,`birthday_from_hours`,`birthday_from_minutes`,`birthday_to_hours`,`birthday_to_minutes`,`status`,`comment`,`number_of_kids`,`number_of_adults`,`company_birthday_data`,`company_location_birthday_slots`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,{$this->user},{$this->user_card},{$this->training_school},{$this->ts_location},'{$this->birthday_date}',{$this->birthday_from_hours},{$this->birthday_from_minutes},{$this->birthday_to_hours},{$this->birthday_to_minutes},'{$this->status}','{$this->comment}','{$this->number_of_kids}','{$this->number_of_adults}',{$this->company_birthday_data},{$this->company_location_birthday_slots},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new company_birthday_reservation($row['id'],$row['user'],$row['user_card'],$row['training_school'],$row['ts_location'],$row['birthday_date'],$row['birthday_from_hours'],$row['birthday_from_minutes'],$row['birthday_to_hours'],$row['birthday_to_minutes'],$row['status'],$row['comment'],$row['number_of_kids'],$row['number_of_adults'],$row['company_birthday_data'],$row['company_location_birthday_slots'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	{
		return "`user` = {$this->user},`user_card` = {$this->user_card},`training_school` = {$this->training_school},`ts_location` = {$this->ts_location},`birthday_date` = '{$this->birthday_date}',`birthday_from_hours` = {$this->birthday_from_hours},`birthday_from_minutes` = {$this->birthday_from_minutes},`birthday_to_hours` = {$this->birthday_to_hours},`birthday_to_minutes` = {$this->birthday_to_minutes},`status` = '{$this->status}',`comment` = '{$this->comment}',`number_of_kids` = '{$this->number_of_kids}',`number_of_adults` = '{$this->number_of_adults}',`company_birthday_data` = {$this->company_birthday_data},`company_location_birthday_slots` = {$this->company_location_birthday_slots},`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
