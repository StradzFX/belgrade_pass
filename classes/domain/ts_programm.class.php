...
class ts_programm implements base_domain_object
{
	public $id = 0;
	...
	public $domain_fields_array = array("id","training_school","ts_location","name","age_from","age_to","trainer");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $training_school = 0, $ts_location = 0, $name = "", $age_from = 0, $age_to = 0, $trainer = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
		...
		$this->name = $name;
		if(is_numeric($age_from)){ $this->age_from = $age_from;}else{$this->age_from = 0;}
		if(is_numeric($age_to)){ $this->age_to = $age_to;}else{$this->age_to = 0;}
	...
	{	
		return "`id`,`training_school`,`ts_location`,`name`,`age_from`,`age_to`,`trainer`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,{$this->training_school},{$this->ts_location},'{$this->name}',{$this->age_from},{$this->age_to},{$this->trainer},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new ts_programm($row['id'],$row['training_school'],$row['ts_location'],$row['name'],$row['age_from'],$row['age_to'],$row['trainer'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
