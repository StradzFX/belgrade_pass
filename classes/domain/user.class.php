...
class user implements base_domain_object
{
	public $id = 0;
	public $email = "";
	public $password = "";
	...
	private $varchar = array("email","password","fb_id","first_name","last_name","avatar","user_type","pib","maticni","adresa","naziv","maker","makerDate","checker","checkerDate","jezik","recordStatus");

	public $domain_fields_array = array("id","email","password","fb_id","first_name","last_name","avatar","user_type","pib","maticni","adresa","naziv");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $email = "", $password = "", $fb_id = "", $first_name = "", $last_name = "", $avatar = "", $user_type = "", $pib = "", $maticni = "", $adresa = "", $naziv = "", $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
	...
	{	
		return "`id`,`email`,`password`,`fb_id`,`first_name`,`last_name`,`avatar`,`user_type`,`pib`,`maticni`,`adresa`,`naziv`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,'{$this->email}','{$this->password}','{$this->fb_id}','{$this->first_name}','{$this->last_name}','{$this->avatar}','{$this->user_type}','{$this->pib}','{$this->maticni}','{$this->adresa}','{$this->naziv}','{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new user($row['id'],$row['email'],$row['password'],$row['fb_id'],$row['first_name'],$row['last_name'],$row['avatar'],$row['user_type'],$row['pib'],$row['maticni'],$row['adresa'],$row['naziv'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	{
		return "`email` = '{$this->email}',`password` = '{$this->password}',`fb_id` = '{$this->fb_id}',`first_name` = '{$this->first_name}',`last_name` = '{$this->last_name}',`avatar` = '{$this->avatar}',`user_type` = '{$this->user_type}',`pib` = '{$this->pib}',`maticni` = '{$this->maticni}',`adresa` = '{$this->adresa}',`naziv` = '{$this->naziv}',`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
