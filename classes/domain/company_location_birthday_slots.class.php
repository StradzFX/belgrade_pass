...
class company_location_birthday_slots implements base_domain_object
{
	public $id = 0;
	public $day_of_week = "";
	public $hours_from = 0;
	...
	public $domain_fields_array = array("id","day_of_week","hours_from","minutes_from","hours_to","minutes_to","price","company_birthday_data");

	private $display_elements_formats = array();
	public $format_options = NULL; 
	
	//Constructor and Domain name
	public function __construct($id = 0, $day_of_week = "", $hours_from = 0, $minutes_from = 0, $hours_to = 0, $minutes_to = 0, $price = 0, $company_birthday_data = 0, $maker = "",$makerDate = "",$checker = "",$checkerDate = "",$pozicija="",$jezik="",$recordStatus="",$modNumber="",$multilang_id = 0)
	{
	...
	{	
		return "`id`,`day_of_week`,`hours_from`,`minutes_from`,`hours_to`,`minutes_to`,`price`,`company_birthday_data`,`maker`,`makerDate`,`checker`,`checkerDate`,`pozicija`,`jezik`,`recordStatus`,`modNumber`,`multilang_id`";
	}

	public function get_attribute_values()
	{
		return "NULL,'{$this->day_of_week}',{$this->hours_from},{$this->minutes_from},{$this->hours_to},{$this->minutes_to},{$this->price},{$this->company_birthday_data},'{$this->maker}','{$this->makerDate}','{$this->checker}','{$this->checkerDate}',{$this->pozicija},'{$this->jezik}','{$this->recordStatus}','{$this->modNumber}','{$this->multilang_id}'";
	}
	
	public function get_object($row)
	{
		$new_object = new company_location_birthday_slots($row['id'],$row['day_of_week'],$row['hours_from'],$row['minutes_from'],$row['hours_to'],$row['minutes_to'],$row['price'],$row['company_birthday_data'],$row["maker"],$row["makerDate"],$row["checker"],$row["checkerDate"],$row["pozicija"],$row["jezik"],$row["recordStatus"],$row["modNumber"],$row["multilang_id"]);
	...
	{
		return "`day_of_week` = '{$this->day_of_week}',`hours_from` = {$this->hours_from},`minutes_from` = {$this->minutes_from},`hours_to` = {$this->hours_to},`minutes_to` = {$this->minutes_to},`price` = {$this->price},`company_birthday_data` = {$this->company_birthday_data},`maker` = '{$this->maker}',`makerDate` = '{$this->makerDate}',`checker` = '{$this->checker}',`checkerDate` = '{$this->checkerDate}',`pozicija` = '{$this->pozicija}',`jezik` = '{$this->jezik}',`recordStatus` = '{$this->recordStatus}',`modNumber` = '{$this->modNumber}',`multilang_id` = '{$this->multilang_id}'";
	}
